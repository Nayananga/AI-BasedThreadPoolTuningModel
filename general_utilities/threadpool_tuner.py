...
def find_next_threadpool_size(target_value, feature_value, trade_off_level, model):
    ...
    if config.SELECTION_METHOD == "Random":
        ...
        threadpool_sizes, max_expected_improvement = generate_min_point_based_on_model(
            target_value, feature_value, model, trade_off_level)

        next_threadpool_size, trade_off_level = select_next_threadpool_size(
            max_expected_improvement, target_value, trade_off_level, threadpool_sizes)
...
def select_next_threadpool_size(max_expected_improvement, target_value, trade_off_level, threadpool_sizes):
