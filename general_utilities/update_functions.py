...
def update_min_data(threadpool_data, target_data, feature_data):
    ...
    if min_location > -1:
        min_threadpool_value = min_threadpool_data[min_location]
        min_target_value = min_target_data[min_location]
        if min_target_value in target_data and min_threadpool_value in threadpool_data:
            if target_data[-1] < min_target_value:
                min_threadpool_data[min_location] = threadpool_data[-1]
                min_target_data[min_location] = target_data[-1]
        else:
            min_threadpool_data.remove(min_threadpool_value)
            min_target_data.remove(min_target_value)
            min_feature_data.remove(feature_value)
    else:
